...
/**
 * SimplePie_GCalendarWT is the SimplePie extension which provides some
 * helper methods.
 *
 * @see http://code.google.com/apis/calendar/docs/2.0/reference.html
 */
class SimplePie_GCalendarWT extends SimplePie {
	 ...
	 * SimplePie_Item_GCalendarWT->get_start_date() returns an empty string,
	 ...
	 * SimplePie_Item_GCalendarWT as item class.
	 * It also sets the variables specified in this feed as query
	 * parameters.
	 */
	function init(){
		$this->set_item_class('SimplePie_Item_GCalendarWT');
 ...
 */
class SimplePie_Item_GCalendarWT extends SimplePie_Item {
	...
	function get_publish_date(){
		if(!$this->gc_pub_date){
			...
			$this->gc_pub_date = SimplePie_Item_GCalendarWT::tstamptotime($pubdate);
		}
	...
	function get_start_date($format = null){
		if(!$this->gc_start_date){
			...
			$this->gc_start_date = SimplePie_Item_GCalendarWT::tstamptotime($startdate);
		}
		if($format != null)
	...
	function get_end_date($format = null){
		if(!$this->gc_end_date){
			...
			$this->gc_end_date = SimplePie_Item_GCalendarWT::tstamptotime($enddate);
		}
		if($format != null)
	 ...
	 * This function can be used to sort an array of SimplePie_Item_GCalendarWT
	 * items with usort.
	 *
